...
import { random, getCommentsCount } from '@/plugin/common'
...
class Manager extends Component {
  ...
  fetchList = ({ current = 1, pageSize = 10, ...query }) => {
    ...
    ArticleService.fetchList({ page: current, pageSize, ...query }).then(res => {
      const pagination = {
        current,
        pageSize,
        total: res.count
  ...
  handleDelete = (articleId, title) => {
    Modal.confirm({
      ...
      content: `文章： ${title} `,
      onOk: () => {
        ArticleService.articleDelete({articleId}).then(res => {
          if (res.code === 200) {
