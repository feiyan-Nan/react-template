...
import { getCommentsCount } from '@/plugin/common'
...
class UserManage extends Component {
  ...
  fetchList = ({ current = 1, pageSize = 10, ...query }) => {
    ...
    UserService.getUserList({ params: { page: current, pageSize, ...query } }).then(res => {
      const pagination = {
        current,
        pageSize,
        total: res.count
  ...
  handleDelete = (userId, username) => {
    Modal.confirm({
      ...
      content: `用户： ${username} `,
      onOk: () => {
        UserService.userDelete({ params: { userId } }).then(res => {
          if (res.code === 200) {
